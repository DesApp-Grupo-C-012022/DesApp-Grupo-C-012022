...
class TransactionConfirmReception : TransactionConfirmBase() {
    override fun doProcess(order: Order, userFromOrder: User, executingUser: User) {
        ...
        mercadoPagoApi.transferMoney(order.totalArsPrice, userFromOrder.mercadoPagoCVU, executingUser.mercadoPagoCVU)
        criptoExchanger.transferCriptoCurrency(order.quantity, order.price.askCurrency.ticker, executingUser.walletAddress, userFromOrder.walletAddress)
    }
}